...
def sync_products_from_supabase():
    try:
        ...
        print(f'Product sync error: {exc}')
        return False
    ...
    try:
        ...
        if synced:
            ...
            for order in json_data['orders']:
                if order.get('order_id') in synced:
                    order['synced'] = True
                    order['synced_at'] = datetime.utcnow().isoformat()
            save_json_data(json_data)
        return True
    except Exception as exc:
        print(f'Order queue sync error: {exc}')
        return False
...
def sync_pending_data_if_possible():
    try:
        if not has_internet():
            return False
        json_data = load_json_data()
        if json_data.get('order_queue'):
            return sync_queued_orders()
        return sync_products_from_supabase()
    except Exception as exc:
        print(f'Pending sync error: {exc}')
        return False
...
def save_order_to_supabase(order_data):
    try:
        json_data = load_json_data()
        json_data.setdefault('orders', [])
        if not any(order.get('order_id') == order_data.get('order_id') for order in json_data['orders']):
        ...
        if not has_internet():
            ...
            if order_data.get('order_id') not in [q.get('order_id') for q in queue]:
                ...
                save_json_data(json_data)
            return {'success': True, 'synced': False, 'queued': True, 'message': 'Order saved offline and will sync when internet returns.'}
        ...
        if order_data.get('order_id') not in [q.get('order_id') for q in queue]:
            ...
            json_data['order_queue'] = queue
            save_json_data(json_data)
        return {'success': True, 'synced': False, 'queued': True, 'message': 'Order queued for sync because the server was unavailable.'}
    except Exception as exc:
    ...
    try:
        ...
        for order in orders:
            ...
            for item in items:
                ...
                product_name = product.get('name') or item.get('name') or f'Product {product_id}'
                ...
                category_name = product.get('category') or item.get('category') or 'Uncategorized'
